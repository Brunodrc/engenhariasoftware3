import {PoupancaInvalidaError, ContaJaCadastradaError, SaldoInsuficienteError, ContaInexistenteError, AplicacaoError, ValorInvalidoError } from "./exceptionsBanco";
...
export class ContaImposto extends Conta {
    ...
    descontarTaxa(valor: number): number {
        return valor * (this.taxaDesconto / 100)
    }
...
export class Banco {
	...
	depositar(numero: String, valor: number): void {
        ...
        if(contaConsultada instanceof ContaImposto){
            contaConsultada.depositar(valor - contaConsultada.descontarTaxa(valor))
        } else {
	...
	sacar(numero: String, valor: number): void {
		let contaConsultada = this.consultar(numero);
		if(contaConsultada instanceof ContaImposto){
            contaConsultada.sacar(valor - contaConsultada.descontarTaxa(valor))    
        } else {
	...
	transferir(numeroDebito: string, numeroCredito: string, valor: number) {
        ...
        if(contaDebito instanceof ContaImposto){
            let valorDescontado = valor - contaDebito.descontarTaxa(valor)
            contaDebito.transferir(contaCredito, valorDescontado);
        } else {
